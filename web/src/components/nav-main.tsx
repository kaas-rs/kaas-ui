...
import type { ClusterCard, EnvironmentSection, Feature, Resource } from "@/api/types";
import {
...
import { chooseEnvironment, pickEnvironment, useChosenEnvironment } from "@/lib/environment";
...
function ClusterItem({ card, active }: { card: ClusterCard; active: boolean }) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} asChild className="group/cluster">
      <SidebarMenuItem>
        <SidebarMenuButton
          asChild
        ...
        {items.length > 0 ? (
          <>
            <CollapsibleTrigger asChild>
              <SidebarMenuAction aria-label={`${open ? "Collapse" : "Expand"} ${card.id}`}>
 ...
 */
function ResourceItem({ resource }: { resource: EnvironmentSection["resources"][number] }) {
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        ...
        tooltip={resource.endpoint ? `${resource.name} — ${resource.endpoint}` : resource.name}
        className="text-sidebar-foreground/55 hover:bg-transparent hover:text-sidebar-foreground/55"
      >
        <div title={resource.endpoint ? `${kind.label} — ${resource.endpoint}` : kind.label}>
          <Icon aria-hidden />
    ...
    <SidebarGroup>
      ...
      <SidebarMenu>
        {active.clusters.map((card) => (
          <ClusterItem key={card.id} card={card} active={card.id === activeCluster} />
        ))}
