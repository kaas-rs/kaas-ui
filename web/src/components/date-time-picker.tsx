...
export function DateTimePicker({
  ...
  const time = value ? formatInTimeZone(value, timeZone, "HH:mm:ss") : "00:00:00";
  ...
  const matcher = React.useMemo(() => {
    const rules: Matcher[] = [];
    if (disabled?.before) rules.push({ before: toZonedTime(disabled.before, timeZone) });
    if (disabled?.after) rules.push({ after: toZonedTime(disabled.after, timeZone) });
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {value ? formatInTimeZone(value, timeZone, "d MMM yyyy") : "Pick a date"}
          </Button>
        </PopoverTrigger>
